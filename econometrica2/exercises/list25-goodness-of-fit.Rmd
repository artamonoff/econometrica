...
for(i in 1:length(regr)) {
  n <- nobs(regr[[i]])
  logL <- -deviance(regr[[i]])/2
  logL.null <- -regr[[i]]$null.deviance/2
  pseudo.R2 <- c(pseudo.R2, 1-logL/logL.null)
  adj.pseudo.R2 <- c(adj.pseudo.R2, 1-(logL-(regr[[i]]$rank-1))/logL.null)
  CoxSnell <- c(CoxSnell, 1-(exp(2*(logL.null-logL)/n)) )
  Nagelkerke <- c(Nagelkerke, (1-(exp(2*(logL.null-logL)/n)))/(1-exp(2*logL.null/n)) )
  ...
  Efron <- c(Efron, efron)
  McKelvey <- c(McKelvey, mckelvey)
}
rm(efron, n)
my.logL.null <- -regr[[1]]$null.deviance/2
```

Для каждой регрессии вычислитк следующие показатели качества подгонки модели: \(R^2_{pseudo}\), \(adj.R^2_{pseudo}\), 
...
Для датасета `TableF5-1.csv` рассморим несколько probit-регрессй. Результаты оценивания
...
Для каждой модели вычислите показатели информационнвх критериев AIC & BIC и \(adj.R^2_{pseudo}\). **Ответ округлите до `r my.digits`-х десятичных знаков.**
