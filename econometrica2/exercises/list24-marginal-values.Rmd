...
- предельные значения для каждого регрессора в средней точке: \(\phi(\bar{x}'\beta)\beta_j\) для probit, \(\lambda(\bar{x}'\beta)\beta_j\) для logit
- среднее по всей выборке предельное значения для каждого регрессора: \(\overline{\phi(x'\beta)\beta_j}\)  для probit, \(\overline{\lambda(x'\beta)\beta_j}\) для logit
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, comment=''}
maBina(regr.probit, x.mean=TRUE)
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, comment=''}
maBina(regr.probit, x.mean=FALSE)
```
